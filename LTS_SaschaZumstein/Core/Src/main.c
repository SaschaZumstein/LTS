...
int main(void)
{
  ...
  /* USER CODE BEGIN WHILE */
	while (1) {
		...
		// error state
		case LTS_ERROR:
			state = error_state(lts_error, bluetoothConnection, usbConnection);
			entry = false;
			break;
		}
    /* USER CODE END WHILE */
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  HAL_GPIO_WritePin(GPIOB, SHUTTER_Pin|READ_Pin|CLR_PIX_Pin, GPIO_PIN_RESET);
